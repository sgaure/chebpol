...
@ 
<<fig.dim=c(5,5)>>=
f <- function(x) 10/(1+25*sum(x^2))
# make non-uniform grid
grid <- list(x=seq(-1,1,len=15), y=seq(-1,1,len=15))
knots <- t(expand.grid(grid))
knots <- knots + rnorm(length(knots),sd=0.05)
...
drawtri <- function(pts,col) {
  segments(pts[1,1],pts[2,1],pts[1,2],pts[2,2], col=col)
  segments(pts[1,1],pts[2,1],pts[1,3],pts[2,3], col=col)
  segments(pts[1,2],pts[2,2],pts[1,3],pts[2,3], col=col)
}
plot(knots[1,],knots[2,],typ='p',pch=20,xlab='x',ylab='y',main='Delaunay triangulation')
invisible(apply(simp,2,function(kn) {drawtri(knots[,kn],col='blue')}))
test <- matrix(runif(2*6,-1,1),2)
rbind(true=apply(test,2,f), sl=g(test), ml=ml(test))
@ 

\subsection{Polyharmonic splines}
There is also an implementation of polyharmonic splines in the method \code{"polyharmonic"}.
