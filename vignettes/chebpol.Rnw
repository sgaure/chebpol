...
values, not the function, we may pass those to \code{ipol} as an arrayd
with the \code{dim} attribute correctly set to match the \code{dims} argument.
...
exception is the method \code{"polyharmonic"} which instead relies on a parallel
