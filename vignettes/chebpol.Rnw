...
of the point.
We create a function defined on a circle, and put some random knots in there, and see what
the Delaunay triangulation looks like.
<<echo=FALSE>>=
set.seed(42)
@ 
<<fig.width=5,fig.asp=1.1,message=FALSE>>=
...
The main shortcoming of this method is visible in the Delaunay triangulation. Some
of the triangles near the borders are very stretched out. This can happen even
if we have a very large number of knots. This is because the triangles
must cover the convex hull, and there is no other way to do that. Such long simplices will
lead to poor linear approximation if second derivatives there are large. By default, the
interpolant will return \code{NA} for inputs which are outside the convex hull of
the knots, but an extrapolation from the nearest simplex will be done if the
argument \code{epol=TRUE} is specified.
