...
are found. This is used in \code{fhappx}. Here is an example with a translated Runge function.
<<lagrange,fig=TRUE,png=TRUE>>=
f <- function(x) 1/(1+25*(x-0.17)^2)
# Uniform grid:
unigrid <- list(seq(-2,2,length.out=20))
uni <- fhappx(f,grid=unigrid,d=2)
...
So the \code{fhappx} creates a Floater-Hormann interpolation on the specified points.
