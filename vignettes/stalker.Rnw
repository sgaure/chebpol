...
with \(1\leq r \leq 2\).  Alternatively, we use a hyperbolic function
\(a+ bx + \frac{c}{1+dx}\).  In this note we call these functions defined by
three points and a monotonicity constraint, \emph{basis functions}. They
are not basis functions in the sense used in the spline literature; they are
...
The dark blue curves are quadratic. The blue curves are monotonic, but with lowered
degree. The light blue curves are non-monotonic with lowered degree.
Every function except for the two obvious ones are
...
to 0. Ideally, the non-monotonic curves should have an extreme point in \(x=0\), i.e.\
no overshoot, but that is impossible with this function form.
...
\(a + bx +d\frac{1}{1+cx}\) which are guaranteed to honour both monotonicity
and local extrema, but like the varying degree stalker it does not generalize well
to higher dimensions, more or less because ``monotonic'' isn't such a simple concept there. 
...
The hyperbolic stalker is specified in chebpol as a stalker with zero degree.
...
\begin{equation}
f(x) = a + bx -\frac{a}{1+cx}.
\end{equation}
When the points are monotonic, i.e. \(\sign(v_-) \neq \sign(v_+)\)
the coefficients \(a,b\) and \(c\) are computed as
\begin{equation}
  \begin{aligned}
    a &= \frac{2v_-v_+}{v_- + v_+}\\
    b &= 0\\
...
\begin{equation}
  \begin{aligned}
    a &= -2v_-v_+\frac{v_-+v_+}{(v_- - v_+)^2}\\
    b &= 2\frac{v_-v_+}{v_- - v_+}\\
    c &= \frac{v_- - v_+}{v_- + v_+}\\
  \end{aligned}
\end{equation}
...
the computation of a dynamic degree involves solving a non-linear equation numerically.
...
We also illustrate the hyperbolic stalker spline.
...
alleviated by using a constant degree.  The hyperbolic stalker is remarkably faithful
to the points, but it also suffers from artefacts in certain areas where it is ill-conditioned.
...
The torsion problem can be seen near the right corner.
We can take a closer look at these portions of the surface in figure~\ref{fig:torsion}.
The striped artefacts are regions where the degree changes.
...
In figure~\ref{fig:constant} is the same part of the surface with hyperbolic stalker
in the first dimension, and constant degree 1.5 in the second.
...
dimension. It is possible to specify any nonzero degree.
A degree of zero is treated specially, with hyperbolic functions \(a+bx+\frac{c}{1+dx}\).
Specifying \code{k=NA} makes the spline adjust the
degree locally as described above. The hyperbolic and varying degree stalker do not generalize
very well to two or more dimensions,
so for multidimenional data varying degree or hyperbolic stalker is strongly discouraged, except
