...
then a proper spline, since it is not a polynomial. We also obtain a limit in
intuitive geometric terms on how large the overshoot can be.
\end{abstract}
\maketitle
...
\code{splinefun(...,method='mono.FC')} also does something similar.
...
If the sharp turns in flat areas is a problem, it is possible
to enforce a minimum degree, e.g. let \(r = \max(1.2,|b/c|)\), no
changes to \(a\), \(b\), and \(c\) are necessary. This avoids the
non-differentiable case for \(|b|=|c|\) as well as the very
sharp turns when \(|b|\) is close to \(|c|\) when two consecutive
values are very close, but it will result in
larger overshoot.

There is a basis function in every knot, so when \(x_2 - x_1 = 1\) and 
\(x_1 \leq x \leq x_2\) we glue the functions \(f_1\) and \(f_2\) together
as a convex combination \(tf_1(x-x_1) + (1-t)f_2(x-x_2)\). By default
we choose \(t = x_2 - x\). It also works by choosing some other \emph{blending function} \(t =t(x)\) as long
as \(t(x_1) = 1\) and \(t(x_2) = 0\). We may e.g.\ use \(t(x) = \cos^2(\frac\pi2 (x-x_1))\).

\section{Examples}
In this section we compare the stalker spline to the 
\code{"natural"} and \code{"mono.FC"} spline from \code{stats::splinefun}.
We also illustrate a stalker spline with minimum degree 1.5.

<<echo=FALSE,fig.dim=c(5.5,5.5)>>=
interp <- function(x,val,pts,mindeg=1,maxdeg=2,bd=1) {
    N <- length(pts)
    i <- findInterval(x,pts)
    if(i == N) return(val[N])
    # compute the share of the interval from the left
    tl <- (x-pts[i])/(pts[i+1]-pts[i])
    tu <- tl-1
    if(i == 1) {
        al <- val[1]
        bl <- val[2]-val[1]
        ...
        al <- val[i]
        cl <- 0.5*(val[i-1]+val[i+1])-val[i]
        bl <- 0.5*(val[i+1]-val[i-1])
    }
    if(i == N-1) {
        au <- val[N]
        bu <- val[N]-val[N-1]
        ...
        au <- val[i+1]
        bu <- 0.5*(val[i+2] - val[i])
        cu <- 0.5*(val[i+2] + val[i]) - val[i+1]
    }

    if(abs(cl) < abs(bl)) {
        pwl <- if(2*abs(cl) >= abs(bl)) abs(bl/cl) else 2
    } else {
        pwl <- abs(cl/bl)
    }
    if(abs(cu) < abs(bu)) {
        pwu <- if(2*abs(cu) >= abs(bu)) abs(bu/cu) else 2
    } else {
        pwu <- abs(cu/bu)
    }
#    message('pw ',round(pwl,2), ' ', round(pwu,2))
    if(is.na(pwl) || pwl > maxdeg) pwl <- maxdeg
    if(pwl < mindeg) pwl <- mindeg
    if(is.na(pwu) || pwu > maxdeg) pwu <- maxdeg
    if(pwu < mindeg) pwu <- mindeg
    vl <- al + bl*tl + cl*abs(tl)^pwl
    vu <- au + bu*tu + cu*abs(tu)^pwu
#    tl <- cospi(0.5*(x-pts[i]))^2
    (1-tl^bd)*vl + tl^bd*vu
}
...
lines(s, sapply(s,interp,val=val,pts=pts),col='magenta')
lines(s, sapply(s,interp,val=val,pts=pts,mindeg=1.5),col='red')
legend('topright',legend=c('stalker','mono','natural','stalker 1.5'),
       fill=c('magenta','green','blue','red'))
@ 

Note that both the stalker and the \code{"mono.FC"} spline honours the
completely flat region between points 4 and 5, but between points 7
and 8 \code{"mono.FC"} has considerable overshoot, even though the
...
will in this case reduce to a linear interpolation. The knots are not
exactly pairwise constant, they differ by \(10^{-16}\). This is sufficient
to make \code{"code.FC"} overshoot.
