...
\begin{equation*}
  ...
  \end{aligned}
\end{equation*}
Noting from equation~\eqref{zeroval} that \(a + d_i + D_i = 0\), we can simplify further.
\begin{equation*}
  \begin{aligned}
...
\begin{equation}\label{dimequations}
  ...
  \end{aligned}
\end{equation}
Again we have two equations in three unknowns \(b_i, c_i, d_i\). To find these
parameters we use a monotonicity or overshoot constraint.
...
bit careful as such a pole easily will create instability.
