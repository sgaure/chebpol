...
\begin{abstract}
  ...
  outlined.  It is designed as a one-dimensional interpolation to be
  ...
  not entirely. There is no fancy theory behind, but it served a
  ...
  alternative name could be \emph{wet paper spline}. \pkg{chebpol}
  contains higher dimensional versions too.  The interpolation isn't a
...
@ 
The blue graph does not look differentiable in 0. It is, but it is not twice differentiable.
...
diagonal of the crosses.  In some extreme cases, additional local
...
not differentiable on the grid lines. This can be discerned as striped
