...
That is, the derivative is not well behaved.
The name comes from the fact that it follows the data frighteningly close, 
though it sometimes seems stupid with little foresight. The 2d-version
could as well be called \emph{wet paper spline}. The interpolation isn't
...
f0spline <- function(f0) { st <- ipol(c(0,f0,1),grid=c(-1,0,1),method='stalker'); st(s)}
for(f0 in v0) {
  points(0,f0,pch=20)
...
depending on whether the degree changes or not. This creates problems
e.g.\ for surfaces with torsion, as we shall see later.

<<failure, echo=FALSE,fig.dim=c(4,4), fig.align='center'>>=
s <- seq(-0.5,0.5,len=100)
plot(range(s),c(-1/3,0.5),pch='',xlab='x',ylab='y')
fmspline <- function(fm) { st <- ipol(c(fm,0,1),grid=c(-1,0,1),method='stalker'); st(s)}
lines(s,fmspline(1/3),col='red',lwd=2,lty=2)
lines(s,fmspline(-1/3),col='red',lwd=2,lty=2)
lines(s,fmspline(0.2),col='yellow',lwd=2)
lines(s,fmspline(-0.2),col='green',lwd=2)
lines(s,fmspline(-0.1),col='blue',lwd=2)
@ 
...
we map it to a uniform grid with a monotonic spline from \code{splinefun(method="hyman")}.
...
The interpolant can be given \code{"mindeg"} and \code{"maxdeg"}, these
can be vectors, one degree limit for each dimension. 

In more than one dimension, torsion can be a problem for stability. It can create
striping artefacts. This can be alleviated by having constant degree (\code{maxdeg=mindeg}) 
...
The stalker spline in higher dimension \(N\) is simplistic. It works on a Cartesian grid.
...
<<random, fig.dim=c(3.5,3.5), fig.pos='!h', fig.align='center',fig.cap='Random surface',out.width='.4\\linewidth',fig.ncol=2,fig.subcap=c('stalker', 'Floater-Hormann','stalker 1.5 constant','thin plate spline')>>=
set.seed(42); N <- 8
...
g <- list(x=seq(grid$x[5],grid$x[8],length=50),y=seq(grid$y[5],grid$y[8],length=50))
par(mar=rep(1,4))
...
lines(x,st(rbind(x,pt),mindeg=c(1,2),maxdeg=2), col='grey')
...
par(mar=rep(1,4))
plot3D::persp3D(g$x, g$y, evalongridV(st,grid=g,mindeg=2), colvar=NULL, lighting=light,
       theta=80, ltheta=60, lphi=45, col='green', axes=TRUE)
plot3D::points3D(rep(subgrid$x,4),rep(subgrid$y,each=4),zval,add=TRUE,colvar=NULL,pch=20)
@ 
