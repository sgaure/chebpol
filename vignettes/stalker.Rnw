...
\(a+ bx + \frac{d}{1+cx}\).  In this note we call these functions defined by
...
which is different from the given value in \((0,0)\).
...
A degree of zero is treated specially, with hyperbolic functions \(a+bx+\frac{d}{1+cx}\).
