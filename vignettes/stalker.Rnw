...
we map the grid to a uniform grid with a rational function \(x\mapsto\frac{\lambda x}{1+\gamma x}\).
...
The interpolant can be given \code{"degree"}, these
can be vectors, a fixed degree for each dimension. 
