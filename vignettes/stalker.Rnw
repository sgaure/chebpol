...
There is a basis function in every knot, so we glue the functions \(f_1\) and \(f_2\) together
as a convex combination \(tf_1(x') + (1-t)f_2(x'-1)\) (\(x'-1\) is the normalized
coordinate for \(f_2\), the basis function in the knot \(x_+\)). We can use a linear blender,
\(t = 1 - x'\), but we may also introduce a \emph{blending function} \(t' =\beta(t)\) as long
as \(\beta(0) = 0\) and \(\beta(1-t) = 1-\beta(t)\), and for smoothness \(\beta'(0) = \beta'(1) = 0\).
The stalker interpolant returned by \code{ipol()} has a logical argument
\code{smooth}. The default is \code{FALSE}
which is \(\beta(t) = t\). If \code{TRUE} is specified, the blending function is
\(\beta(t) = \sin^2(\pi t /2)\).
...
will in this case reduces to a linear interpolation. The knots below are not
...
st1.5s <- function(x) st(x,mindeg=1.5,smooth=TRUE)
