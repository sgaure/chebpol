...
\begin{abstract}
  ...
  contains higher dimensional versions too, but only a limited set are useful.  The
...
legend('topleft',legend=c('linear','cubic','sigmoid','parodic','square'),
       fill=c('blue','black','green','red','cyan'))
@ 
...
spline will typically have features parallel with the axes. We consider the
...
\end{equation}
Recall that both \(b_i\) and \(c_i\) are functions of \(r_i\), so it is not
easy to find a closed form solution in terms of Lambert's \(W\) or similar.

For a uniform grid, i.e.\ with \(k_i^- + k_i^+ = 0\), this problem has a closed form solution, otherwise we solve
it numerically.  [fill in details].

\subsection{Non-monotonic points}
For non-monotonic triples have adopted the following strategy. We change the sign of \(v_i^-\). This
...
with a hyperbola.  We consider it in higher dimensions, specifically
\(\RN^n\).  We have a Cartesian product grid, and in every grid point
there should live a function. We will try to honor monotonicity and
extreme points \emph{along the axes}. That is, we do not consider the
grid to be just any points which happen to lie on a grid, but as
...
Incidentally, there are some plane areas, and
some torsion. Torsion is a problem for the stalker spline, the degree
in one dimension goes down to 1, creating striped artefacts in some
regions. This can be alleviated by using a constant degree. The multidimensional stalker
will be replaced at a later time. The
hyperbolic stalker is much better.  There is no advanced shading in
\code{plot3D::persp3D}, so the resolution can be
seen if you zoom in.

<<random, fig.dim=c(3.5,3.5), fig.pos='!ht', fig.align='center',fig.cap='Random surface',out.width='.5\\linewidth',fig.ncol=2,fig.subcap=c('general', 'thin plate spline','hyperbolic stalker','Floater-Hormann')>>=
set.seed(52); N <- 8
...
gn <- ipol(val,grid=grid, method='general')
