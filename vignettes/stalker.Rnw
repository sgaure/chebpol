...
the computation of a dynamic degree involves solving a non-linear equation.
...
<<random, fig.dim=c(3.5,3.5), fig.pos='!h', fig.align='center',fig.cap='Random surface',out.width='.4\\linewidth',fig.ncol=2,fig.subcap=c('stalker', 'thin plate spline','stalker degree 1.5','Floater-Hormann')>>=
set.seed(42); N <- 8
...
<<constant, fig.dim=c(5,5), out.width='\\linewidth',fig.cap='Constant degree 2', fig.align='center'>>=
par(mar=rep(1,4))
...
dimension. Incidentally, it is possible to specify any constant degree, though negative ones
creates creased surfaces. Degree 0 is treated specially, with hyperbolic basis functions.
Specifying \code{k=NA} makes the spline adjust the
degree locally as described above. The varying degree stalker do not tensor very well,
so it should be used with care for multidimensional
data. Evaluation on a non-uniform grid currently involves solving a non-linear equation
