...
with constant curvature, but not very visible with wobbly
data.
...
We take a look at 2d-interpolation, first the Maungawhau volcano with exaggerated height.
...
Then we interpolate some random points. Incidentally, there are some
plane areas, and some torsion (most visible closest to the
camera). Torsion where the tangent plane passes the horizontal plane
can be a problem for the stalker spline, the degree in one dimension
goes down to 1, creating striped artefacts in some regions. This is
alleviated by using a larger minimum degree.  There is no advanced
shading in \code{plot3D::persp3D}, so the \(100\times 100\) resolution
can be seen if you zoom in.
<<fig.dim=c(3.5,3.5), fig.pos='!h', fig.align='center',fig.cap='Random surface',out.width='.4\\linewidth',fig.ncol=2,fig.subcap=c('stalker','stalker max 1.2', 'stalker min 2','thin plate spline')>>=
set.seed(42); N <- 8
...
st1 <- function(x) st(x,maxdeg=1.2)
st2 <- function(x) st(x,mindeg=2)
ph <- ipol(val,grid=grid, method='polyharmonic',k=2)
g <- list(x=seq(0,1, len=100), y=seq(0,1,len=100))
par(mar=rep(0,4))
for(f in list(st, st1, st2, ph)) {
  plot3D::persp3D(g$x, g$y, evalongridV(f,grid=g), colvar=NULL, lighting=light,
         theta=135, ltheta=110, lphi=40, col='green', axes=FALSE, bty='n', scale=FALSE,zlim=c(0,1))
}
@ 
