...
\begin{abstract}
  ...
  monotonicity is achieved by somewhat non-traditional means, by
  ...
  alternative name could be ``wet paper spline''. \pkg{chebpol}
...
we modify the quadratic function by lowering the degree:
...
In this note we call these functions defined by
three points and a monotonicity constraint, \emph{basis functions}. They
are not basis functions in the vector space sense; they are
...
The stalker spline can also be realized with hyperbolic functions of the type
...
both monotonicity and local extrema. In a special case, it reduces to a parabola.
We get back to this in the higher dimensional case.
...
more than one of the coordinates are non-zero. Thus, the dependence on \(i\) can be
dropped in the following equations, though we do keep it to avoid confusion.
...
to a neighbouring basis function.
...
points, it's not actually interpolating.  In order to have reasonably
...
there. Linear blending creates a continuous interpolation, but it is
...
<<meanblend, fig.cap='2d blending', fig.dim=c(4,4),out.width='0.4\\linewidth',echo=FALSE,fig.ncol=2,fig.subcap=c('mean','linear','cubic','sigmoid')>>=
set.seed(49)
NN <- 3
...
light <- list(specular=0.2,ambient=0.0,diffuse=0.6)
for(bl in c('mean','linear','cubic','sigmoid')) {
  par(mar=rep(0,4))
  plot3D::persp3D(g$x, g$y, evalongridV(ip,grid=g,blend=bl), colvar=NULL, lighting=light,
                  theta=135, ltheta=100, lphi=40, col='green',bty='n',axes=FALSE,zlim=range(value))
...
such a concoction escapes my topological intuition.
...
legend('topright',legend=c('stalker','mono','natural','hyperbolic'),
       col=c('magenta','green','blue','red'),lty=1)
@ 
