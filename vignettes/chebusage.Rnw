...
\([-1,1]^2\) and level plots of it and the \(9 \times 9\) Chebyshev
...
It is not so easy to see in this picture, but the stalker interpolant
is not generally twice differentiable in the grid points. The other
interpolants are typically twice continuously differentiable.
