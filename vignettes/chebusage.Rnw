...
of it and the \(8 \times 8\) Chebyshev interpolation, and the multilinear and Floater-Hormann methods on
...
\(O((2n)^r\log n)\). The evaluation of Chebyshev interpolants is not
...
<<fast>>=
f <- ipol(sin, grid=seq(0,1,1000), method='multi')
a <- runif(1e6)
system.time(sin(a))
