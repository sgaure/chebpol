...
Here is a 3d-picture of the function \(f(x,y) = 4/(4+(x+y)^2)\) on \([-5,5] \times [-5,5]\) 
with a perturbed \(15 \times 15\) grid.
<<fig.align='center', fig.dim=c(5,5)>>=
f <- function(x) 1 / (1+mean(x^2))
GN <- 15
D <- 5
grid <- list(x=sort(seq(-D,D,len=GN)+c(0,rnorm(GN-2,sd=D/GN),0)), 
             y=sort(seq(-D,D,len=GN)+c(0,rnorm(GN-2,sd=D/GN),0)))
fh <- ipol(f, grid=grid, method='fh')
knots <- t(expand.grid(grid))
sl <- ipol(f, knots=knots, method='simplex')
ph <- ipol(f, knots=knots, method='poly')
g <- list(x=seq(-5,5,len=100), y=seq(-5,5,len=100))

par(mfrow=c(2,2),mar=c(0,0,0,0))
light <- list(specular=0.8,ambient=0,diffuse=0.4)
col <- 'green'
plot3D::persp3D(g$x, g$y, evalongrid(f,grid=g), colvar=NULL, lighting=light,
        theta=0, ltheta=-30, lphi=45, col=col, axes=FALSE, bty='n',zlim=c(0,1))
plot3D::persp3D(g$x, g$y, evalongridV(ph,grid=g), colvar=NULL, lighting=light,
        theta=0, ltheta=-30, lphi=45, col=col, axes=FALSE, bty='n',zlim=c(0,1))
plot3D::persp3D(g$x, g$y, evalongridV(fh,grid=g), colvar=NULL, lighting=light,
        theta=0, ltheta=-30, lphi=45, col=col, axes=FALSE, bty='n',zlim=c(0,1))
plot3D::persp3D(g$x, g$y, evalongridV(sl,grid=g,epol=TRUE), colvar=NULL, lighting=light,
        theta=0, ltheta=-30, lphi=45, col=col, axes=FALSE, bty='n',zlim=c(0,1))
@ 

% \begin{center}
% \includegraphics[width=0.8\textwidth]{rungetest.png}
% \end{center}
...
tuning. The three parameters in \code{k} is the base radius, the number of layers and
a smoothing parameter \(\lambda\). The ALGLIB docs says that the base radius should be
