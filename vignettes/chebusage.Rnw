...
\title{Usage note for package \pkg{chebpol}}
...
pick our knots exactly as we wish, e.g. as a grid \(X \times Y = \{(x,y) \,|\, x \in X \text{ and } y \in Y\}\)
for some \(X\) and \(Y\)?
...
traditional Chebyshev interpolation. 
<<cheb,fig=TRUE>>=
...
is the degree of certain blending polynomials. In theory it works with any degree up to the
...
All of the above methods work with more than one dimension, but that presumes
that the function can be evaluated on a Cartesian grid. The Chebyshev and uniform methods
use particular grids. The method \code{"general"} is similar to \code{"uniform"} in
that it transforms an arbitrary Cartesian grid into a Chebyshev grid by means of a monotonic spline
in each dimension.
The multi linear and Floater-Hormann methods also work on arbitrary Cartesian grids.

To illustrate how this works, here is a quite nice function on \([-1,1]^2\) and level plots
of it and the \(8 \times 8\) Chebyshev interpolation, and the multi linear and Floater-Hormann methods on
irregular \(8 \times 8\) grids.

<<echo=FALSE>>=
set.seed(44)
@ 
<<dim2,fig=TRUE,include=FALSE>>=
f1 <- function(x) 1.5/log(5+sin(pi/2*(x[1]-x[2]^2)))
ch1 <- ipol(f1, dims=c(8,8), method='cheb')
igrid <- list(x=seq(-1,1,len=8)^3, y=sort(runif(8,-1,1)))
...
lattice::levelplot(m.lin+F.H.+fun+cheb ~ x+y, data=data, cuts=100,
          col.regions=grey.colors(101,0.1,0.9), layout=c(2,2), colorkey=FALSE,
          main='Level plots of function and interpolations')
@ 
\begin{center}
\includegraphics[width=16cm,height=16cm]{chebusage-dim2}
\end{center}
...
<<poly,fig=TRUE,include=FALSE>>=
f2 <- function(x) 1/min(log(1.9 + sin(0.7+1.8*pi*(x+0.39*x[1]^2-x[2]^2))^2))
ph1 <- ipol(f1, knots=matrix(runif(70,-1,1),2), method='polyharmonic', k=3)
...
lattice::levelplot(fun2+poly2+fun1+poly1 ~ x+y, data=data, cuts=100,
          col.regions=grey.colors(101,0.1,0.9), layout=c(2,2), colorkey=FALSE,
          main='Level plots of functions and interpolations')
@ 

\begin{center}
\includegraphics[width=16cm,height=16cm]{chebusage-poly}
\end{center}
...
points \(n\) along a dimension, or the number of knots \(K\). I.e. for
...
\((2n)^r\log n\). The evaluation of Chebyshev interpolants is not
...
The multi linear interpolation is the fastest in \pkg{chebpol}. Creating
...
Typically a \(\log n\) task for each dimension, but there are \(2^r\) corners for an \(r\)-dimensional
problem, so \(2^r r\log n\).

Creation of the Floater-Hormann interpolation requires calculating the weights,
roughly an \(rn^2\) task. Evaluation is \(n^r\). Quite similar to the Chebyshev method.
...
\(K\times K\) linear system, a \(K^3\) task (or \(n^{3r}\) to compare with the grid methods).
Evaluation is faster, of the order \(Kr\) (or \(rn^r\)).
